...
export default function (notifyNoResultsElement, notifyBadRequestElement, resultListContainerElement, searchResultTemplateElement, config, startIndex, queryString, callback) {
  const searchResultsPromise = getJSONResults(`https://www.googleapis.com/customsearch/v1` + `?key=${config.apiKey}` + `&cx=${config.engineKey}` + `&q=${queryString}&hl=en&start=${startIndex}`);
