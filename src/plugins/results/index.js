...
export default function(settings) {
  ...
  const [searchResultTemplateElement] = document.getElementsByClassName(config.searchResultTemplateClassName);

  // TODO: refactor this into separate 'initial results' plugin
  ...
  if (queryString !== null && queryString !== '') {
    ...
    // TODO: refactor the parameters (they shouldn't all be set each time)
    ...
    initResults(noResultsElement, resultListContainerElement, searchResultTemplateElement, apiKey, engineKey, startIndex, queryString, function(response) {
      ...
      // TODO: register custom event on this location, use this in the pagination function block below
      if (enablePagination) {
  ...
  if (enablePagination) {
    ...
    if (elementExists(paginationContainer)) {
      paginationContainer.addEventListener(
        ...
        function(e) {
          if (
            e.target.classList.contains(config.nextButtonClassName) ||
            e.target.classList.contains(config.previousButtonClassName)
          ) {
